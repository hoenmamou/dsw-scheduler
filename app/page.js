...
const clientProfile = useMemo(() => (state.clients || []).find(c => c.id === clientProfileId) || null, [state.clients, clientProfileId]);
const clientProfileShifts = useMemo(() => (state.shifts || []).filter(sh => sh.clientId === clientProfileId), [state.shifts, clientProfileId]);
const clientProfileStaff = useMemo(() => (state.staff || []), [state.staff]);
const clientProfileHours = useMemo(() => {
  let totalMin = 0, dayMin = 0, nightMin = 0;
  for (const sh of clientProfileShifts) {
...
{clientProfile ? (
  <div style={{ position: "fixed", inset: 0, display: "grid", placeItems: "center", background: "rgba(0,0,0,0.6)", zIndex: 1000 }}>
    <div style={{ width: 800, maxWidth: "98%", ...styles.card }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ margin: 0 }}>{clientProfile.name} Profile</h2>
        <button style={styles.btn2} onClick={() => setClientProfileId("")}>Close</button>
      </div>
      <div style={{ marginTop: 10 }}>
        <b>Supervisor:</b> {(state.users || []).find(u => u.id === clientProfile.supervisorId)?.name || "Unassigned"}<br />
        <b>Coverage:</b> {clientProfile.coverageStart} – {clientProfile.coverageEnd}<br />
        <b>Weekly Allotment:</b> {clientProfile.weeklyHours}h
      </div>
      <div style={{ marginTop: 18 }}>
        <h3>Schedule (All Shifts)</h3>
        <div style={{ maxHeight: 220, overflowY: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {clientProfileShifts.map(sh => (
                <tr key={sh.id}>
                  <td style={styles.td}>{sh.startISO.replace("T", " ").slice(0, 16)}</td>
                  <td style={styles.td}>{sh.endISO.replace("T", " ").slice(0, 16)}</td>
                  <td style={styles.td}>{clientProfileStaff.find(s => s.id === sh.staffId)?.name || "Unknown"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ marginTop: 18 }}>
        ...
        <div>Total: {fmtHoursFromMin(clientProfileHours.totalMin)} | Day: {fmtHoursFromMin(clientProfileHours.dayMin)} | Night: {fmtHoursFromMin(clientProfileHours.nightMin)}</div>
      </div>
    </div>
  </div>
) : null}
